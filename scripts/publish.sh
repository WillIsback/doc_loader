...
verify_publication() {
    ...
    case $platform in
        "pypi")
            ...
            pip install extracteur-docs-rs --no-cache-dir
            python -c "import doc_loader; print('✅ Import réussi')"
            
            deactivate
            rm -rf "$verify_venv"
            print_success "Package PyPI vérifié"
            ;;
        "crates")
            print_info "Vérification crates.io..."
            sleep 10  # Attendre la propagation
            
            cargo search doc_loader | grep "doc_loader"
            print_success "Package crates.io vérifié"
            ;;
    esac
}
