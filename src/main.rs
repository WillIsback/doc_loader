...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let matches = Command::new("Doc Loader")
        .version("1.0.0")
        .about("Universal document processor for multiple file formats")
        .arg(
            Arg::new("input")
                .short('i')
                .long("input")
    ...
    // Process the file using the universal processor
    println!("Processing file: {}", input_file);
    let processor = UniversalProcessor::new();
    
    let result = match processor.process_file(input_path, Some(params)) {
        Ok(output) => output,
        Err(e) => {
    ...
    eprintln!("   📋 Type: {}", result.document_metadata.document_type.to_string());
